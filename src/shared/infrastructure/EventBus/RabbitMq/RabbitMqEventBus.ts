...
export default class RabbitMqEventBus implements EventBus {
      ...
      async message => {
        ...
        if (event) {
          ...
          if (subscribers && subscribers.length) {
            ...
            this.logger.log(`[RabbitMqEventBus] Message processed: ${event.eventName} by ${subscribersNames}`);
  ...
  async publish(events: Array<DomainEvent>): Promise<void> {
    ...
    events.map(event => {
      ...
      this.logger.log(`[RabbitMqEventBus] Event to be published: ${event.eventName}`);
